                ...
                if (dataLine.size() != 0) { // check if empty line
                    if (!dataLine.get(0).startsWith("%")) { // check if
                        ...
                        for (int i = 0; i < dataLine.size(); i++) {
                            ...
                            if (i == userCol) {
                                String userId = col;
                                int rowValue = userIds.containsKey(userId) ? userIds.get(userId) : userIds.size();
                                userIds.put(userId, rowValue);
                            } else if (i == itemCol) {
                                String itemId = col;
                                int colValue = itemIds.containsKey(itemId) ? itemIds.get(itemId) : itemIds.size();
                                itemIds.put(itemId, colValue);
                            }
